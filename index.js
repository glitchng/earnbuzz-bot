...
const ADMIN_PASS = process.env.ADMIN_PASS || "letmein"; // set on Render
...
function getRandomNigerianName() {
  const firstNames = [/* same list */];
  const lastNames = [/* same list */];
...
app.use(session({
  secret: 'super_secret_key', // replace in production
  resave: false,
    ...
    <body class="flex items-center justify-center h-screen">
      <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm text-center">
        ...
        <label class="flex items-center justify-center space-x-2">
          <span class="text-gray-700">Broadcasting</span>
          <input type="checkbox" ${broadcasting ? 'checked' : ''} onchange="toggleBroadcast(this)" class="w-6 h-6 text-blue-600 rounded">
        </label>
        <p class="mt-4 text-sm text-gray-500" id="status">${broadcasting ? 'Running' : 'Stopped'}</p>
